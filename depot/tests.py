...
class ProductTest(TestCase):
    ...
    ####  title，description，price，imageUrl不能为空
    ...
    ####   imageUrl必须以jpg，png，jpg结尾，并且对大小写不敏感；
    def test_imgae_url_endwiths(self):
        ...
        for endwith in oks:
            self.product['imageUrl'] = url_base+endwith
            f = ProductForm(self.product)
            self.assertTrue(f.is_valid(),msg='error when imageUrl endwith '+endwith)

        for endwith in bads:
            self.product['imageUrl'] = url_base+endwith
            f = ProductForm(self.product)
            self.assertFalse(f.is_valid(),msg='error when imageUrl endwith '+endwith)

        self.product['imageUrl'] = 'http://google.com/logo.png'
